...
function ScoreStrip({ game, viewedPlayerId, onInspect }: { game: GameState; viewedPlayerId: string; onInspect: (playerId: string) => void }) {
  return <div className="score-strip" aria-label="플레이어 공개 현황">{game.players.map((player) => <button type="button" key={player.id} className={`score-player${player.id===game.currentPlayerId?' current':''}${player.id===viewedPlayerId?' viewing':''}`} onClick={()=>onInspect(player.id)} aria-pressed={player.id===viewedPlayerId} aria-label={`${player.name}님의 패 보기`}><span className="score-color" style={{background:player.color}}>{player.name.slice(0,1)}</span><span className="score-copy"><b>{player.name}</b><small>공개 {scorePlayer(game,player.id)}점 · 자원 {bagTotal(player.resources)}장</small></span>{game.longestRoadOwner===player.id&&<em title="최장 교역로">길 +2</em>}{game.largestArmyOwner===player.id&&<em title="최대 기사단">기사 +2</em>}</button>)}</div>;
}
...
export default function GameApp() {
  const [game,setGame]=useState<GameState|null>(null); const [activeSlot,setActiveSlot]=useState<SaveSlot|null>(null); const [panel,setPanel]=useState<'build'|'trade'|'cards'|'log'>('build'); const [buildMode,setBuildMode]=useState<BuildMode>(null); const [help,setHelp]=useState(false); const [settings,setSettings]=useState(false); const [saveOpen,setSaveOpen]=useState(false); const [viewedPlayerId,setViewedPlayerId]=useState<string|null>(null);
  ...
  useEffect(()=>setViewedPlayerId(null),[game?.currentPlayerId]);
  useEffect(()=>{
  ...
  const player=game.players.find((entry)=>entry.id===game.currentPlayerId)!;
  const viewedPlayer=game.players.find((entry)=>entry.id===viewedPlayerId)??player;
  ...
  return <main className="game-shell">
    ...
    <ScoreStrip game={game} viewedPlayerId={viewedPlayer.id} onInspect={(playerId)=>{setViewedPlayerId(playerId===game.currentPlayerId?null:playerId);setPlayerPanelCollapsed(false);setBuildMode(null)}}/><PhaseGuide game={game}/>
    <div className={`game-layout${playerPanelCollapsed?' panel-collapsed':''}`}>
      <section className="board-column">
        <GameBoard game={game} buildMode={buildMode} act={act}/>
        {playerPanelCollapsed&&<button className="player-panel-expand" type="button" aria-expanded="false" onClick={()=>setPlayerPanelCollapsed(false)}><span aria-hidden="true">‹</span><b>{viewedPlayer.name}님 패 열기</b></button>}
      </section>
      {!playerPanelCollapsed&&<aside className="player-panel">
        <div className="private-title"><span className="player-token" style={{background:viewedPlayer.color}}>{viewedPlayer.name.slice(0,1)}</span><div><b>{viewedPlayer.name}님의 패</b><small>실제 {scorePlayer(game,viewedPlayer.id,true)}점 · 공개 {scorePlayer(game,viewedPlayer.id)}점</small></div><button className="player-panel-collapse" type="button" aria-expanded="true" onClick={()=>setPlayerPanelCollapsed(true)}><span aria-hidden="true">›</span><small>패널 접기</small></button></div>
        <div className="resource-row">{RESOURCES.map((resource)=><ResourcePill key={resource} resource={resource} amount={viewedPlayer.resources[resource]}/>)}</div>
        <div className="piece-stats"><span>길 {viewedPlayer.roads.length}/15</span><span>최장 {longestRoadLength(game,viewedPlayer.id)}</span><span>기사 {viewedPlayer.usedKnights}</span><span>카드 {viewedPlayer.devCards.filter((c)=>!c.played).length}</span></div>
        {viewedPlayer.id!==player.id?<div className="other-hand-view"><span className="player-token" style={{background:player.color}}>{player.name.slice(0,1)}</span><div><b>{viewedPlayer.name}님의 패를 보는 중</b><small>게임 행동은 {player.name}님의 패로 돌아간 뒤 할 수 있어요.</small></div><button type="button" onClick={()=>setViewedPlayerId(null)}>{player.name}님 패로 돌아가기</button></div>:<>{game.phase==='pre-roll'&&<div className="roll-zone"><DicePair dice={animatedDice} rolling={rolling}/><span className="roll-status" aria-live="polite">{rolling?'주사위가 굴러가는 중입니다':'두 주사위의 눈을 확인하세요'}</span>{preRollKnight&&<button className="pre-roll-knight" disabled={rolling} onClick={()=>act({type:'PLAY_DEV',cardId:preRollKnight.id})}>기사 먼저 사용하기</button>}<button className="roll-button" disabled={rolling} onClick={roll}>{rolling?'굴리는 중…':'주사위 굴리기'}</button></div>}{game.phase==='robber-steal'&&<div className="victim-list"><h3>자원을 가져올 상대</h3>{game.eligibleVictims.map((id)=><button key={id} onClick={()=>steal(id)}>{game.players.find((candidate)=>candidate.id===id)?.name} · 무작위 1장</button>)}</div>}{game.phase==='main'&&<><nav className="panel-tabs">{([['build','건설'],['trade','거래'],['cards','개발'],['log','기록']] as const).map(([id,label])=><button key={id} className={panel===id?'active':''} onClick={()=>{setPanel(id);setBuildMode(null)}}>{label}</button>)}</nav><div className="action-panel">{panel==='build'&&<BuildPanel game={game} mode={buildMode} setMode={setBuildMode}/>} {panel==='trade'&&<TradePanel game={game} act={act}/>} {panel==='cards'&&<CardPanel game={game} act={act} setBuildMode={setBuildMode}/>} {panel==='log'&&<div className="log-list">{visibleLogs.map((entry)=><p key={entry.id}>{entry.message}</p>)}</div>}</div><button className="end-turn" disabled={game.freeRoads>0||Boolean(game.pendingTrade)} onClick={()=>{resetTurnUi();setPanel('build');setBuildMode(null);act({type:'END_TURN'})}}>차례 마치기 →</button></>}</>}
      </aside>}
    </div>
