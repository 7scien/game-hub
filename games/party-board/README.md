...
- 사용자가 그린 경로를 기준으로 고정 제작한 항구 마을 보드(서쪽 오르막, 상단 부두 고리, 중앙 가로길, 정원 대각길, 해변 마을 고리)
- 글로벌 60턴 milestone과 누적 월급 규칙
- 밝은 회백색 석재 판석, 매립형 원형 칸, 낮은 옹벽·잔디·관목·수로로 구성한 Three.js 로우폴리 3D 보드
- 넓어진 항구 마을 스케일과 칸 중심 간 최소 시각 간격 검증으로 경로·칸 겹침 방지
- 큰길과 실제 geometry로 연결되는 부두 안쪽길·마을 세로길·정원 대각길·해변 마을길
...
2. `supabase/migrations/202608260001_phase1_rooms.sql`을 적용합니다.
   기존 프로젝트에 첫 migration을 이미 적용했다면 새 경로 저장 형식을 위한 `supabase/migrations/202608270001_harbor_village_board.sql`도 이어서 적용합니다.
