...
기존 Phase 1 migration을 이미 적용한 프로젝트는 다음 추가 migration을 파일명 순서대로 한 번씩 실행한다.

1. `202608270001_harbor_village_board.sql`
2. `202608270002_curved_harbor_board.sql`
3. `202608270003_online_minigame.sql`

세 번째 파일은 기존 방·저장 데이터를 삭제하지 않고 미니게임 이벤트 테이블, READY, 결과 집계, 1회 보상 및 보드 복귀 RPC를 추가한다.
...
운영 빌드는 Chrome 프로필 네 개 또는 Chrome·Edge·Firefox의 서로 다른 일반 프로필처럼 저장공간이 분리된 창 네 개로 확인한다. 로컬 개발 서버에서는 `?profile=p1`, `?profile=p2`, `?profile=p3`, `?profile=p4`를 각각 붙인 탭 네 개를 사용할 수 있다. 개발 빌드에서만 Supabase Auth 저장 키가 분리되므로 한 브라우저에서도 서로 다른 익명 사용자 네 명으로 동작한다.
...
### G. 테스트 미니게임 온라인 파이프라인

1. 현재 플레이어가 `온라인 턴 완료`를 눌러 글로벌 턴을 진행한다. 턴 6을 마칠 때 네 화면이 모두 `별빛 부두 포착전` 설명 화면으로 전환되는지 확인한다.
2. 네 플레이어가 각자 `READY`를 누른다. 각 화면에 같은 READY 4/4 상태가 표시되는지 확인한다.
3. 네 화면에서 같은 서버 시각을 기준으로 3·2·1·GO가 진행되는지 확인한다.
4. 각 탭에서 별빛을 서로 다른 횟수만큼 누른다. 다른 플레이어 화면에는 점수만 전파되고 로컬 입력이나 경기장 조작은 영향을 받지 않는지 확인한다.
5. 진행 도중 한 탭을 새로고침한다. 같은 READY·COUNTDOWN·남은 경기 시간·서버 점수로 복구되는지 확인한다.
6. 종료 후 네 화면의 점수·순위·승자가 같은지 확인한다. 네 명이 결과 확인을 누르면 우승자의 코인이 20에서 60으로 한 번만 바뀌고 `minigameWins`가 1 증가해야 한다.
7. 결과 화면을 새로고침하거나 결과 확인 요청을 재시도해도 코인이 추가로 지급되지 않는지 확인한다.
8. `REWARD_APPLIED → RETURNING_TO_BOARD → BOARD` 순서로 네 화면이 복귀하고 글로벌 턴 7과 다음 플레이어가 같게 표시되는지 확인한다.
...
- 네 화면의 미니게임 인스턴스·READY·카운트다운·최종 결과가 동일하다.
- 서버 입력 이벤트 집계 결과만 승자와 보상을 결정한다.
- 새로고침과 중복 요청 후에도 40코인 보상과 미니게임 승리 수가 한 번만 반영된다.
- 네 화면이 같은 다음 글로벌 턴으로 복귀한다.
