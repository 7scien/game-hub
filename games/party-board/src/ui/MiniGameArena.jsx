...
export function MiniGameArena({active,onScore,initialScore=0,playerSeat=0}){
  ...
  useEffect(()=>{
    const scene=new StarCatchScene({canvas:canvasRef.current,playerSeat,onReady:()=>setReady(true),onScore:value=>onScoreRef.current?.(value),onError:reason=>setError(reason?.message||'미니게임 경기장을 열 수 없습니다.')});
    sceneRef.current=scene;return ()=>{sceneRef.current=null;scene.dispose()};
  },[playerSeat]);
  useEffect(()=>{sceneRef.current?.setActive(active,initialScore)},[active,initialScore]);
