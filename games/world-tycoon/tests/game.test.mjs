...
test('2~4인 게임을 데이터 기반 보드와 함께 생성한다',()=>{
  for(const count of [2,3,4]){
    ...
    assert.equal(state.players.length,count);assert.equal(state.board.length,28);assert.equal(state.phase,PHASES.WAITING_FOR_ROLL);
...
test('출발점을 통과하면 보너스를 받고 도착한 도시를 살 수 있다',()=>{
  const state=createGame(2,{mode:'full',rng:()=>.2});const player=state.players[0];player.position=27;
  ...
  buyCurrentTile(state);assert.equal(state.board[1].ownerId,player.id);assert.ok(player.ownedProperties.includes('seoul'));
});
...
test('지역 완성과 시설 묶음 보너스가 통행료에 반영된다',()=>{
  ...
  const rail=state.board.find(tile=>tile.id==='rail');assert.equal(calculateRent(state,rail,visitor),rail.baseRent*RULES.FACILITY_MULTIPLIERS[4]);
});

test('상대 도시에 도착하면 통행료가 자동 정산된다',()=>{
  const state=createGame(2,{mode:'full',rng:()=>.2});const owner=state.players[0];const visitor=state.players[1];const tile=state.board[22];
  tile.ownerId=owner.id;owner.ownedProperties.push(tile.id);state.currentPlayerIndex=1;visitor.position=22;
...
test('건물 없는 자산과 현금을 플레이어끼리 거래한다',()=>{
  ...
  openTrade(state);proposeTrade(state,{partnerId:'player-2',offerCash:100,requestCash:50,offerAssetId:'seoul',requestAssetId:''});resolveTrade(state,true);
...
test('지불 능력이 없으면 파산하고 마지막 플레이어가 승리한다',()=>{
  const state=createGame(2,{mode:'full',rng:()=>.2});const owner=state.players[0];const debtor=state.players[1];const tile=state.board[22];
  tile.ownerId=owner.id;tile.buildingLevel=4;owner.ownedProperties.push(tile.id);state.currentPlayerIndex=1;debtor.position=22;debtor.money=0;
