...
function playerStrip(state){
  return `<section class="player-strip" aria-label="플레이어 현황">${state.players.map((player,index)=>`<article class="player-mini ${index===state.currentPlayerIndex?'current':''} ${player.bankrupt?'bankrupt':''}" style="--player-color:${player.color}"><i>${player.token}</i><span><b>${escapeHtml(player.name)}</b><small>${player.bankrupt?'파산':`${formatMoney(player.money)} · 자산 ${formatMoney(getNetWorth(state,player.id))}${getLoanBalance(player)?` · 대출 ${formatMoney(getLoanBalance(player))}`:''}`}</small></span></article>`).join('')}</section>`;
}
...
function bankPanel(state){
  ...
  return `<section class="bank-panel"><div class="section-label"><span>SEED BANK</span><b>₩</b></div>${loan?`<div class="loan-summary"><span><small>대출 원금</small><b>${formatMoney(loan.principal)}</b></span><span><small>이자 10%</small><b>${formatMoney(loan.interest)}</b></span><span><small>만기까지</small><b>${Math.max(0,loan.dueLap-player.lapsCompleted)}바퀴</b></span></div><button class="bank-repay-button" type="button" data-action="repay-bank-loan" ${!available||player.money<balance?'disabled':''}>전액 상환 ${formatMoney(balance)}</button>`:'<p class="bank-copy">최대 100만 원 · 고정 이자 10% · 3바퀴 만기</p>'}${remainingCapacity>=100000?`<form data-loan-form><input name="loanAmount" type="number" min="100000" max="${remainingCapacity}" step="10000" value="${Math.min(1000000,remainingCapacity)}" aria-label="은행 대출 금액"><button type="submit" ${available?'':'disabled'}>대출받기</button></form>`:''}</section>`;
}
