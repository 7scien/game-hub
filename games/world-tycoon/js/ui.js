...
import {SPECIAL_CARD_INFO,canUseBank,getAuctionBidder,getBuildableOwnedCities,getCurrentPlayer,getGlobalEffectRounds,getLoanBalance,getNetWorth,getTradeableAssets,getUnownedPurchasableAssets,isGlobalEffectActive} from './game.js';
...
const phaseLabel={
  ...
  [PHASES.BUY_DECISION]:'인수 결정',[PHASES.BUILD_DECISION]:'도시 개발',[PHASES.BUILD_ANYWHERE_DECISION]:'보유 도시 자유 건설',[PHASES.AUCTION]:'잔여 자산 경매',[PHASES.TRAVEL_DECISION]:'게임판에서 목적지 선택',[PHASES.WORLD_CUP_DECISION]:'월드컵 개최 도시 선택',[PHASES.TERROR_TARGET_DECISION]:'파괴할 건물이 있는 도시 선택',[PHASES.PAYMENT_DECISION]:'통행료 지불 방법 선택',[PHASES.TRADE]:'플레이어 거래',[PHASES.ASSET_MANAGEMENT]:'자산 정리',[PHASES.END_TURN]:'턴 마무리',[PHASES.GAME_OVER]:'게임 종료',
};
...
function boardMarkup(state){
  ...
  return `<div class="board-grid">${board.map((tile,index)=>{
    ...
    const side=tileSide(index);const travelTarget=state?.phase===PHASES.TRAVEL_DECISION&&tile.type==='city';const terrorTarget=state?.phase===PHASES.TERROR_TARGET_DECISION&&tile.type==='city'&&tile.buildingLevel>0;const auctionTarget=state?.phase===PHASES.AUCTION&&tile.id===state.auction?.currentTileId;const occupationTarget=imperialActive&&koreanTiles.has(tile.id);const transportLocked=rageActive&&(tile.type==='facility'||tile.id==='space-travel');
...
export function renderStart(root,{savedGames=[],setup=false,playerCount=2,selectedSlot=1}={}){
  ...
  root.innerHTML=previewLayout(`<aside class="control-panel setup-card"><p class="eyebrow">SAVE ${selectedSlot} · PASS &amp; PLAY</p><h2>새로운 여행을<br>시작할까요?</h2><p class="setup-copy">전반전에는 토지만 매입하고, 잔여 5개 경매 뒤 후반전부터 도시를 개발합니다.</p>
...
function playerStrip(state){
  return `<section class="player-strip" aria-label="플레이어 현황">${state.players.map((player,index)=>`<article class="player-mini ${index===state.currentPlayerIndex?'current':''} ${player.bankrupt?'bankrupt':''}" data-player-mini="${player.id}" style="--player-color:${player.color}"><i>${player.token}</i><span><b>${escapeHtml(player.name)}</b><small>${player.bankrupt?'파산':`${formatMoney(player.money)} · 자산 ${formatMoney(getNetWorth(state,player.id))}${getLoanBalance(player)?` · 대출 ${formatMoney(getLoanBalance(player))}`:''}`}</small></span></article>`).join('')}</section>`;
}
...
export function renderHelp(){
  return `<div class="modal-backdrop free-modal" role="dialog" aria-modal="true"><section class="help-card"><button class="modal-close" type="button" data-action="close-free-modal" aria-label="닫기">×</button><p class="eyebrow">HOW TO PLAY</p><h2>World Tycoon 가이드</h2><div class="help-grid">${[
    ...
    ['💾','저장 슬롯','서로 다른 게임 2개가 이 기기에 각각 자동 저장됩니다. 시작 화면에서 이어할 게임을 선택합니다.'],['◈','전반전','건설 없이 토지만 매입하며 상대 땅에서는 대지 통행료만 냅니다. 미분양 자산이 5개가 되면 전반전이 끝납니다.'],
    ['🔨','하프타임 경매','남은 자산 5개를 차례대로 공개 경매합니다. 최고 입찰자가 낙찰받고 모든 경매가 끝나면 후반전이 시작됩니다.'],['▥','후반전 건설','주사위를 굴리기 전에는 땅을 직접 밟지 않아도 어느 보유 도시든 원하는 만큼 개발할 수 있습니다.'],
    ...
    ['🏦','은행 대출','자기 차례에 원금 합계 최대 100만 원까지 빌릴 수 있습니다. 고정 이자 10%이며 대출 뒤 3바퀴 안에 전액 상환하지 못하면 파산합니다.'],['!','파산','현금이 부족하면 건물과 자산을 반값에 정리합니다. 모두 정리해도 못 내거나 대출 만기를 지키지 못하면 파산합니다.'],['♛','승리','완전 게임은 마지막 생존자가, 시간제 게임은 종료 시 대출을 뺀 순자산 1위가 승리합니다.']
