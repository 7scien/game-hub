...
const city=(id,name,englishName,region,purchasePrice,{buildable=true,fixedRent=null}={})=>({
  ...
  buildingCosts:buildable?[.55,.9,1.15,1.15,1.15].map(rate=>roundTenThousand(purchasePrice*rate)):[],
  rentByLevel:buildable?[.11,.32,1.35,2.45,3.55,4.65].map(rate=>roundTenThousand(purchasePrice*rate)):[fixedRent],
  ...LANDMARKS[id],
});
