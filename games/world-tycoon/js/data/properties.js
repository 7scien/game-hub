...
const city=(id,name,englishName,region,purchasePrice,{buildable=true,fixedRent=null}={})=>({
  ...
  buildingCosts:buildable?[.55,.7,.9,1.15].map(rate=>roundTenThousand(purchasePrice*rate)):[],
  rentByLevel:buildable?[.11,.32,.7,1.35,2.45].map(rate=>roundTenThousand(purchasePrice*rate)):[fixedRent],
});
